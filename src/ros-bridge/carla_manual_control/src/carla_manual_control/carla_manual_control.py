...
dp_ratio = 0.8   # change display size
...
class HUD(object):
    ...
    def __init__(self, role_name, width, height, node):
        ...
        font = pygame.font.Font(pygame.font.get_default_font(), int(20 * dp_ratio))             # change display size
        fonts = [x for x in pygame.font.get_fonts() if 'mono' in x]
        default_font = 'ubuntumono'
        mono = default_font if default_font in fonts else fonts[0]
        mono = pygame.font.match_font(mono)
        # self._font_mono = pygame.font.Font(mono, 14)
        self._font_mono = pygame.font.Font(mono, int(14* dp_ratio))     # change display size
        # self._notifications = FadingText(font, (width, 40), (0, height - 40))
        self._notifications = FadingText(font, (width, int(40 * dp_ratio)), (0, height - int(40 * dp_ratio)))   # change display size
        # self.help = HelpText(pygame.font.Font(mono, 24), width, height)
        self.help = HelpText(pygame.font.Font(mono, int(24 * dp_ratio)), width, height)    # change display size
    ...
    def render(self, display):
        ...
        if self._show_info:
            ...
            info_surface = pygame.Surface((int(240 * dp_ratio), self.dim[1]))   # change display size
            ...
            for item in self._info_text:
                if int((v_offset + 18) * dp_ratio) > self.dim[1]:
                    break
                if isinstance(item, list):
                    if len(item) > 1:
                        points = [(int((x + 8) * dp_ratio), int((v_offset + 8 + (1.0 - y) * 30) * dp_ratio)) for x, y in enumerate(item)]
                        # pygame.draw.lines(display, (255, 136, 0), False, points, 2)
                        pygame.draw.lines(display, (255, 136, 0), False, points, int(2 * dp_ratio)) # change display size
                    item = None
                    v_offset += 18
                elif isinstance(item, tuple):
                    if isinstance(item[1], bool):
                        # rect = pygame.Rect((bar_h_offset + 50, v_offset + 8), (6, 6))
                        rect = pygame.Rect((int((bar_h_offset + 50) * dp_ratio), int((v_offset + 8) * dp_ratio)), (int(6 * dp_ratio), int(6 * dp_ratio)))    # change display size
                        pygame.draw.rect(display, (255, 255, 255), rect, 0 if item[1] else 1)
                    else:
                        # rect_border = pygame.Rect((bar_h_offset, v_offset + 8), (bar_width, 6))
                        rect_border = pygame.Rect((int(bar_h_offset * dp_ratio), int((v_offset + 8) * dp_ratio)), (int(bar_width * dp_ratio), int(6 * dp_ratio)))   # change display size
                        pygame.draw.rect(display, (255, 255, 255), rect_border, 1)
                        f = (item[1] - item[2]) / (item[3] - item[2])
                        if item[2] < 0.0:
                            # rect = pygame.Rect((bar_h_offset + int(f * (bar_width - 6)), v_offset + 8), (6, 6))
                            rect = pygame.Rect((int((bar_h_offset + int(f * (bar_width - 6))) * dp_ratio), int((v_offset + 8) * dp_ratio)), (int(6 * dp_ratio), int(6 * dp_ratio))) # change display size
                        else:
                            # rect = pygame.Rect((bar_h_offset, v_offset + 8), (int(f * bar_width), 6))
                            rect = pygame.Rect((int(bar_h_offset * dp_ratio), int((v_offset + 8) * dp_ratio)), (int(f * bar_width * dp_ratio), int(6 * dp_ratio)))   # change display size
                        pygame.draw.rect(display, (255, 255, 255), rect)
                    item = item[0]
                if item:  # At this point has to be a str.
                    surface = self._font_mono.render(item, True, (255, 255, 255))
                    # display.blit(surface, (8, v_offset))
                    display.blit(surface, (int(8 * dp_ratio), int(v_offset * dp_ratio)))    # change display size
                v_offset += 18
...
class FadingText(object):
    ...
    def set_text(self, text, color=(255, 255, 255), seconds=2.0):
        ...
        self.surface.blit(text_texture, (int(10 * dp_ratio), int(11 * dp_ratio)))   # change display size
...
class HelpText(object):
    ...
    def __init__(self, font, width, height):
        ...
        self.dim = (int(680 * dp_ratio), int((len(lines) * 22 + 12) * dp_ratio))     # change display size
        ...
        for n, line in enumerate(lines):
            ...
            self.surface.blit(text_texture, (22 * dp_ratio, n * 22 * dp_ratio)) # change display size
            self._render = False
...
def main(args=None):
    ...
    resolution = {"width": int(800 * dp_ratio), "height": int(600 * dp_ratio)}  # change display size
